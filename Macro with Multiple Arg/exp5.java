...
public class MacroExpansion {
    public static void main(String[] args) {
        ...
        // Extract macro name and arguments from macro definition using regex
        String macroName = macroInstructions.get(1).split("\\s+")[1];
        List<String> macroArguments = Arrays.asList(macroInstructions.get(1).split("\\s+")[2].split(","));
        ...
        for (String instruction : sourceCodeInstructions) {
            if (instruction.startsWith(macroName)) {
                ...
                // Expand macro
                for (int i = 1; i < macroInstructions.size() - 1; i++) {
        ...
        // Output statistics
        System.out.println("\nStatistical output:");
        System.out.println("Number of instructions in input source code (excluding Macro calls) = " + (sourceCodeInstructions.size() - macroCalls));
        ...
        for (int i = 0; i < macroCalls; i++) {
            System.out.println("Actual argument during Macro call \"" + macroName + "\" = " + actualArguments.get(i));
        }
        System.out.println("Total number of instructions in the expanded source code = " + totalInstructions);
    }
}
